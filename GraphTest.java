...
public class GraphTest {
    ...
    private void testPlace() {
        ...
        t.new OneTest(c, 10, "Simple test") {
            ...
            public void test() {
                ...
                checkThrows(() -> new Place(3, 10, 8),
                        IllegalArgumentException.class, "y too large");
                checkThrows(() -> new Place(10, 3, 8),
                        IllegalArgumentException.class, "x too large");
            }
        };
    }
    ...
    private void testGraph() {
        Class<?> c = Graph.class;
        t.new AccessChecker(c).pM("connected").pM("addEdge").pM("addVertex")
                .pM("hasEdge").checkAll();
        t.new OneTest(c, 10, "Basic test") {
            ...
            public void test() throws GraphException {
                ...
                checkThrows(() -> g.addVertex("A"), GraphException.class,
                        "Adding an existing vertex");
                checkThrows(() -> g.addEdge("B", "C"), GraphException.class,
                        "Adding an existing edge");
                checkThrows(() -> g.addEdge("B", "F"), GraphException.class,
                        "Adding an edge with non-existent vertex");
                check(g.connected("A", "C"), "connected 1");
                check(g.connected("C", "A"), "connected 2");
                check(!g.connected("D", "A"), "connected 3");
                checkThrows(() -> g.connected("A", "W"), GraphException.class,
                        "connected() with non-existent vertex");
            }
        };
    }
